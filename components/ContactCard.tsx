...
const ContactCard: React.FC<ContactCardProps> = ({ contact }) => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const { saveContact } = await import('../services/geminiService');
    ...
    <div className="bg-gray-800 shadow-lg rounded-xl p-6 mt-8 w-full max-w-2xl mx-auto border border-gray-700">
      <div className="flex justify-between items-start mb-4">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 ${isSaved
